## Executive Summary & Transition from Milestone 1
...
## Data Sources, Ownership, and Constraints

### Data Source and Description
The primary data source utilized for this project is the Hindi subset of the **IndicQA** benchmark, created by **AI4Bharat**. 
...
* **Data Characteristics:** The dataset is composed of structural triples: context paragraphs, question strings, and ground-truth answer spans.

### Ownership, Licensing, and Usage Constraints
...
## Synthetic Generation & LLM Distillation Process
...
### API Security Policy Filtering & Data Attrition
...
## Alignment Between Tasks (Generative to Classification)
...
### Handling Alignment Limitations
...
### Concrete Alignment and Distillation Examples
...
*   **Aligned Tokens & Binary Labels:**
    *   `"आवासीय"` → **1** (Preserved)
    *   `"तथा"` → **1** (Preserved)
    *   `"प्रशासकीय"` → **1** (Preserved)
    *   `"इमारते"` → **1** (Preserved)
    *   `"हैं"` → **1** (Preserved)
    *   `"जिसे"` → **1** (Preserved)
    *   `"दौलतखाना"` → **1** (Preserved) *(Contains answer)*
    *   `"कहते"` → **1** (Preserved)
    *   `"हैं।"` → **1** (Preserved)
...
## Dataset Description & Feature Distribution

### Dataset Size and Split Metrics
...
### Token Lengths and Compression Ratios
...
### Class Feature Distribution (Imbalance Analysis)
...
## Dataset Quality and Pipeline Resiliency

### Assessment of Dataset Quality & Anomalies
...
### Engineering Resiliency Controls
* **Exponential Backoff:** The distillation pipeline handles rate limits (HTTP 429) and timeouts dynamically:
  $$T_{\text{wait}} = T_{\text{base}} \times 2^{\text{attempt}} \quad (T_{\text{base}} = 10\text{s})$$
...
## Dataset Adequacy & Augmentation Strategy
...
### Augmentation and Expansion Options
...
3. **Data Augmentation:** Applying back-translation (Hindi → English → Hindi) on the questions or performing random synonym replacement (using Hindi WordNet) to augment the existing 987 samples.
...
### Split Ratio
The compiled dataset of 987 records is divided into:
* **Train split:** 789 samples (80%).
* **Validation split:** 198 samples (20%).

### Document-Level Data Leakage Prevention
In reading comprehension benchmarks like IndicQA, multiple different questions are generated from the same underlying Wikipedia context article. 
...
* **Mitigation:** The split is executed at the **article level**. The dataset is first grouped by the unique source Wikipedia articles or distinct context paragraphs. The shuffle is performed on these article groupings using a fixed random seed (`seed = 42`) to guarantee experimental reproducibility. All questions corresponding to a specific Wikipedia article are allocated to either the training set or the validation set exclusively. This ensures that the validation set contains completely unseen contexts, forcing the model to generalize.

---

## RAG-Specific Preparations, Chunking, & Vector Databases
...
### Embedding Models & Vector Database Options
...
## Student Model Training & Prompt Formatting

The generated dataset is structured specifically for training a student sequence labeling model:

### Input Representation
The student model (e.g., `xlm-roberta-large`) will ingest the question and context as a single concatenated sequence structure:
```text
...
## Reproducibility & Preprocessing Guide
